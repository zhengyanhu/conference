...
export const datePicker = (function() {
    ...
    MobileCalendar.prototype = {
        init: function(params) {
            ...
            if(params.minuteStep) {
            	this.minuteStep = params.minuteStep;
            }
            if(params.defaultTime) {
                this.defaultTime = params.defaultTime;
            }else{
        ...
        bindEvent: function(type) {
            ...
            function timeCtrlInit() {
                console.log(_self.defaultTime.split(":")[0])
                var d = new Date();
                var e = {
                    hh: parseInt(_self.defaultTime.split(":")[0]),
                    mm: parseInt( _self.defaultTime.split(":")[1])
                };
                ...
                var temp = Math.ceil(e.mm / _self.minuteStep) * _self.minuteStep;
                if(temp > 59) {
                	e.hh += 1;
                	e.mm = "00";
                }else {
            ...
            function setTimeGearTooth() {
                ...
                if (time_mm && time_mm.getAttribute("val")) {
                    ...
                    time_mm.innerHTML = i;
                    time_mm.style["-webkit-transform"] = 'translate3d(0,' + (8 - Math.round(mmVal / _self.minuteStep)* 2) + 'em,0)';
                    time_mm.setAttribute('top', 8 - Math.round(mmVal / _self.minuteStep) * 2 + 'em');
                } else {
                    return
                }
            }
                ...
                target["int_" + target.id] = setInterval(function() {
                    ...
                    switch (target.dataset.datetype) {
                        ...
                        case "time_mm":
                            ...
                            if(pos < 10 - (60 / _self.minuteStep) * 2) {
                            	pos = 10 - (60 / _self.minuteStep) * 2;
                            	stopGear = true;

                            }
                            if (stopGear) {
                                var gearVal = Math.abs(pos - 8) / 2 * _self.minuteStep;
                                setGear(target, gearVal);
                                clearInterval(target["int_" + target.id]);
                            }
                            break;
            ...
            function finishMobileDateTime(e) {
                ...
                time_mm = Math.round(Math.round(time_mm) / _self.minuteStep) * _self.minuteStep;
            ...
            function finishMobileTime(e) {
                ...
                time_hh = time_hh > 9 ? time_hh : '0' + time_hh;
                var time_mm = parseInt(Math.round(_self.gearDate.querySelector(".time_mm").getAttribute("val")));
                var temp = Math.ceil(time_mm / _self.minuteStep) * _self.minuteStep;
                if(temp > 59) {
                	time_hh += 1;
                	time_mm = "00";
                }else {
